#!/usr/bin/env bash
# =============================================================================
# tests/list-status-stale.sh - Stale-container surfacing in `dc list`/`dc status`.
#
...
# after `dc rebuild-image` rebuilds the tag to a new id while the container
# keeps running on the old one until `dc rebuild-container` is run.
#
...
# owner-only dir (chmod 700), as dc_load_project_config requires.
# ---------------------------------------------------------------------------
...
CONTAINER_IMAGE="dev-img-fresh:latest"
...
printf 'fresh\n' >> "$STATE/running"
printf 'fresh\n' >> "$STATE/exists"
printf 'dev-img-fresh:latest\tsha-fresh-aaa\n' >> "$STATE/images"
...
CONTAINER_BACKEND="docker"
CONTAINER_IMAGE="dev-img-stale:latest"
CONTAINER_OVERLAY_SCOPES="nodejs"
CFG
chmod 700 "$DC_ROOT/stale"
chmod 600 "$DC_ROOT/stale/config"
printf 'stale\n' >> "$STATE/exists"
printf 'dev-img-stale:latest\tsha-stale-new\n' >> "$STATE/images"
...
CONTAINER_BACKEND="docker"
CONTAINER_IMAGE="dev-img-miss:latest"
CONTAINER_OVERLAY_SCOPES="nodejs"
CFG
chmod 700 "$DC_ROOT/missing"
chmod 600 "$DC_ROOT/missing/config"
printf 'dev-img-miss:latest\tsha-miss-aaa\n' >> "$STATE/images"
...
CONTAINER_BACKEND="docker"
CONTAINER_IMAGE="dev-img-unk:latest"
CONTAINER_OVERLAY_SCOPES="nodejs"
CFG
...
# ---------------------------------------------------------------------------
# dc list: rightmost WARN column shows STALE only for the stale project.
# ---------------------------------------------------------------------------
LIST_OUT="$WORK/list.out"
run_with_stub "$ROOT_DIR/scripts/list.sh" >"$LIST_OUT" 2>&1 \
  || fail "dc list exited non-zero
$(cat "$LIST_OUT")"

# Header carries the new WARN column.
grep -Eq '^[[:space:]]*NAME[[:space:]].*WARN[[:space:]]*$' "$LIST_OUT" \
  || fail "dc list: header missing WARN column
$(cat "$LIST_OUT")"

# Only the stale project is marked STALE; the other three are not.
grep -Eq '^stale[[:space:]].*STALE[[:space:]]*$' "$LIST_OUT" \
  || fail "dc list: stale project should carry STALE warning
$(cat "$LIST_OUT")"
! grep -Eq '^fresh[[:space:]].*STALE' "$LIST_OUT" \
  || fail "dc list: fresh project must not be marked STALE
$(cat "$LIST_OUT")"
! grep -Eq '^missing[[:space:]].*STALE' "$LIST_OUT" \
  || fail "dc list: missing project must not be marked STALE
$(cat "$LIST_OUT")"
! grep -Eq '^unknown[[:space:]].*STALE' "$LIST_OUT" \
  || fail "dc list: unknown project must not be marked STALE
$(cat "$LIST_OUT")"
# The missing project is reported missing (sanity: the stub state is wired up).
grep -Eq '^missing[[:space:]]+missing[[:space:]]' "$LIST_OUT" \
  || fail "dc list: missing project should show 'missing' state
$(cat "$LIST_OUT")"

pass "dc list: STALE warned only for proven image-id drift"

# ---------------------------------------------------------------------------
# dc status: dedicated stale section lists only the stale project, with the
# rebuild-container remediation hint.
# ---------------------------------------------------------------------------
STATUS_OUT="$WORK/status.out"
run_with_stub "$ROOT_DIR/scripts/status.sh" >"$STATUS_OUT" 2>&1 \
  || fail "dc status exited non-zero
$(cat "$STATUS_OUT")"

grep -Eq '^Stale containers:' "$STATUS_OUT" \
  || fail "dc status: missing 'Stale containers:' section
$(cat "$STATUS_OUT")"
grep -Eq '^[[:space:]]+- stale \(run: dc rebuild-container stale\)' "$STATUS_OUT" \
  || fail "dc status: stale project not listed with rebuild hint
$(cat "$STATUS_OUT")"
! grep -Eq 'rebuild-container fresh' "$STATUS_OUT" \
  || fail "dc status: fresh project must not appear in stale remediation
$(cat "$STATUS_OUT")"
! grep -Eq 'rebuild-container missing' "$STATUS_OUT" \
  || fail "dc status: missing project must not appear in stale remediation
$(cat "$STATUS_OUT")"
! grep -Eq 'rebuild-container unknown' "$STATUS_OUT" \
  || fail "dc status: unknown project must not appear in stale remediation
$(cat "$STATUS_OUT")"

pass "dc status: stale section lists only the stale project"
...
run_with_stub "$ROOT_DIR/scripts/status.sh" >"$STATUS_OUT2" 2>&1 \
  || fail "dc status (after fix) exited non-zero
$(cat "$STATUS_OUT2")"
grep -Eq '^Stale containers:[[:space:]]*$' "$STATUS_OUT2" || {
  ...
  grep -Eq 'none' "$STATUS_OUT2" \
    || fail "dc status: expected 'none' after fixing drift
$(cat "$STATUS_OUT2")"
}
! grep -Eq 'rebuild-container stale' "$STATUS_OUT2" \
  || fail "dc status: stale project should no longer appear after fix
$(cat "$STATUS_OUT2")"

pass "dc status: reports 'none' once drift is resolved"
