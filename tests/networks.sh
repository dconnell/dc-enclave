#!/usr/bin/env bash
# =============================================================================
# tests/networks.sh - Internal-networking feature coverage.
#
# Exercises the full networking stack without a real daemon, using stub CLIs
# (docker/container/podman) that log every call and answer the read predicates
# (image ls, network ls, ps) from controlled files. Mirrors the stub harness in
# tests/new-container-lifecycle.sh.
#
# Coverage:
...
case "$me" in
  ...
  container)
    ...
    # apple `container ps -a` -> first column is the id/name
    if [[ "${1:-}" == "ps" ]]; then [[ -f "$_ctrs" ]] && cat "$_ctrs"; exit 0; fi
    ;;
esac
exit 0
STUB
