...
#
# Covers:
#   - dc_timezone_name_is_valid   (IANA-name charset; rejects shell metachars)
#   - dc_timezone_from_localtime_file (symlink parse, macOS + Linux shapes)
#   - dc_host_timezone            (TZ env honored, invalid rejected, fallback)
# =============================================================================
set -euo pipefail
...
pass "dc_timezone_name_is_valid (IANA charset, rejects shell metachars)"

# ---------------------------------------------------------------------------
# dc_timezone_from_localtime_file (symlink parsing)
# ---------------------------------------------------------------------------
# Linux shape: /usr/share/zoneinfo/<Area>/<Location>
ln -s "/usr/share/zoneinfo/America/Los_Angeles" "$WORK/linux_lt"
got="$(dc_timezone_from_localtime_file "$WORK/linux_lt")" \
...
got="$(dc_timezone_from_localtime_file "$WORK/mac_lt")" \
...
got="$(dc_timezone_from_localtime_file "$WORK/three_lt")" \
...
: > "$WORK/copy_lt"
if dc_timezone_from_localtime_file "$WORK/copy_lt" >/dev/null 2>&1; then
  fail "localtime: regular file must NOT resolve"
fi

# Missing path -> fail (not crash).
if dc_timezone_from_localtime_file "$WORK/does_not_exist" >/dev/null 2>&1; then
...
if dc_timezone_from_localtime_file "$WORK/bad_lt" >/dev/null 2>&1; then
  fail "localtime: non-zoneinfo symlink must NOT resolve"
fi

pass "dc_timezone_from_localtime_file (Linux/macOS shapes, rejections)"

# ---------------------------------------------------------------------------
# dc_host_timezone (selection order: TZ env -> /etc/localtime -> none)
# ---------------------------------------------------------------------------
# Explicit, valid TZ env is honored verbatim.
got="$(TZ=America/New_York dc_host_timezone)" \
  || fail "host_tz: valid TZ env must succeed"
[[ "$got" == "America/New_York" ]] \
  || fail "host_tz: TZ env not honored (got [$got])"

got="$(TZ=UTC dc_host_timezone)" || fail "host_tz: UTC env"
...
if out="$(TZ='bad;rm -rf tz' dc_host_timezone 2>/dev/null)"; then
...
if out="$(TZ='America/New York' dc_host_timezone 2>/dev/null)"; then
  fail "host_tz: whitespace TZ must be rejected"
fi
[[ -z "$out" ]] || fail "host_tz: whitespace TZ must emit nothing (got [$out])"

pass "dc_host_timezone (TZ env honored, invalid rejected)"
