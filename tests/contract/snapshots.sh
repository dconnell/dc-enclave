...
# --- apple: container inspect <name> (--format {{.ImageID}}, or bare JSON) ---
if [[ "\$me" == "container" && "\${1:-}" == "inspect" && "\${2:-}" != "image" ]]; then
  fv="\$(fmt_val "\$@")"
  if [[ -z "\$fv" ]]; then
    # no --format: emit JSON (image reference) for jq-based config parsing.
    printf '[{"configuration":{"image":{"reference":"sha256:base-image"}}}]\n'
    exit 0
  fi
...
# Unified apple handler: size / id / label lookup / config fields; bare (no
# --format) emits JSON variants[].config.config for jq-based config parsing.
if [[ "\$me" == "container" && "\${1:-}" == "image" && "\${2:-}" == "inspect" ]]; then
  ref="\${3:-}"; fv="\$(fmt_val "\$@")"
  if [[ -z "\$fv" ]]; then
    printf '[{"variants":[{"config":{"config":{"user":"dev","WorkingDir":"/workspace","Env":["PATH=/usr/local/sbin:/usr/local/bin"],"Cmd":["/usr/local/bin/entrypoint"],"Entrypoint":[]}}}]}]\n'
    exit 0
  fi
  case "\$fv" in
    '{{.size}}') printf '2048000\n'; exit 0 ;;
...
# apple/container has no 'ps' subcommand; it uses 'ls' (-a includes stopped,
# -q prints clean names). Same state files as the ps branch above.
if [[ "\${1:-}" == "ls" ]]; then
  all=\$([[ " \$* " == *" -a "* ]] && echo yes || echo no)
  if [[ "\$all" == yes ]]; then src="\$_exists"; else src="\$_running"; fi
  awk 'NF' "\$src" 2>/dev/null
  exit 0
fi
...
# The apple commit path reads image config via jq (optional dep); skip cleanly
# in environments where jq is not installed.
if ! command -v jq >/dev/null 2>&1; then
  echo "SKIP: backend_container_commit (apple) requires jq (optional dep)"
else
BACKEND=apple
: > "$LOG"
...
grep -Fq "CALL container inspect appleproj" "$LOG" \
  || fail "apple: must inspect container for its image reference"
grep -Fq "CALL container image inspect sha256:base-image" "$LOG" \
  || fail "apple: must inspect base image config"
grep -Fq "CALL container export -o " "$LOG" || fail "apple: must export container FS"
grep -Eq "CALL container (build|.*build)" "$LOG" \
...
# Regression: with NO snapshots present, REMOVE_SNAPS is empty, which used to
# make the empty-check evaluate ${#REMOVE_SNAPVOLS[@]:-0} (an invalid
# substitution -> "bad substitution", exit 1). It must exit 0 and report none.
run_script "$ROOT_DIR/scripts/clean.sh" --snapshots --dry-run \
  >"$WORK/clean_empty.stdout" 2>"$WORK/clean_empty.stderr" \
  || fail "dce clean --snapshots (no snapshots) exited non-zero
-- stderr:$(cat "$WORK/clean_empty.stderr")"
grep -Fqi "No snapshots found" "$WORK/clean_empty.stdout" \
  || fail "clean --snapshots (no snapshots): should report 'No snapshots found'"
pass "dce clean --snapshots (no snapshots): exits 0, reports none (no bad-substitution)"
