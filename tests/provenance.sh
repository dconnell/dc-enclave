...
# overlays/ leaf dir. content_hash takes the leaf dir; dc_log_provenance takes
...
# ---------------------------------------------------------------------------
# dc_provenance_content_hash (per-side, takes EFFECTIVE scopes)
# ---------------------------------------------------------------------------
mkfrag team nodejs
ch1="$(dc_provenance_content_hash "$TEAM_OD" nodejs)"
[[ "$ch1" =~ ^[0-9a-f]{12}$ ]] || fail "content_hash: expected 12 hex (got [$ch1])"
ch2="$(dc_provenance_content_hash "$TEAM_OD" nodejs)"
[[ "$ch1" == "$ch2" ]] || fail "content_hash: not deterministic"
printf 'RUN echo CHANGED\n' > "$TEAM_OD/Containerfile.nodejs"
ch3="$(dc_provenance_content_hash "$TEAM_OD" nodejs)"
...
mkfrag team golang
ab="$(dc_provenance_content_hash "$TEAM_OD" all,nodejs,golang)"
ba="$(dc_provenance_content_hash "$TEAM_OD" all,golang,nodejs)"
...
dc_log_provenance "$PROJECT" "dev-img-deadbeefdeadbeef:latest" "new" "$TEAM_ROOT" "$USER_ROOT" "nodejs" "sha256:baseAAA"
...
dc_log_provenance "$PROJECT" "dev-img-deadbeefdeadbeef:latest" "rebuild" "$TEAM_ROOT" "$USER_ROOT" "nodejs" "sha256:baseAAA"
...
dc_log_provenance "$PROJECT" "dev-img-deadbeefdeadbeef:latest" "rebuild" "$TEAM_ROOT" "$USER_ROOT" "nodejs" "sha256:baseBBB"
...
if command -v git >/dev/null 2>&1 && command -v jq >/dev/null 2>&1; then
  ...
  dc_log_provenance "$PROJECT" "dev-img-1111222233334444:latest" "new" "$TEAM_ROOT" "$USER_ROOT" "nodejs" "sha256:baseZZZ"
