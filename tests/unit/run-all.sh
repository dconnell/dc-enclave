#!/usr/bin/env bash
# =============================================================================
# tests/unit/run-all.sh - Fast/stubbed unit + contract suite.
#
# Single entrypoint that runs every test file in tests/unit/. These are fast,
# deterministic command/contract tests that stub the container runtime (no real
# daemon). Real-backend end-to-end coverage lives separately under
# tests/integration/ and is never run from here.
#
# `smoke.sh` is intentionally excluded (it chains these files AND adds the
