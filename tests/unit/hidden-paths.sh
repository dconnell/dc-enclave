...
expect_invalid() {
  local value="$1"
  if dce_normalize_hidden_paths_csv "$value" >/dev/null 2>&1; then
...
expect_valid() {
  ...
  local expected="$2"
  local actual=""
  actual="$(dce_normalize_hidden_paths_csv "$value")" || fail "expected valid hidden path: $value"
...
vol_a="$(dce_hidden_volume_name "MyProject" "node_modules")"
vol_b="$(dce_hidden_volume_name "MyProject" "apps/web/node_modules")"
vol_c="$(dce_hidden_volume_name "MyProject" "node_modules")"
