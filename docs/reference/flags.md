...
| `--sync-ignore <path[,path...]>` | Adjust the Mutagen ignore set; re-creates the sync session so the new rules apply. Same grammar as the `dce new` flag. Only meaningful with `--sync` (or an already-synced project). |
