...
- **A per-project log** — `~/.config/dce-enclave/<name>/provenance.jsonl` (JSON Lines, owner-only) appends one entry per distinct image state, so the history survives the `:latest` overwrite. It is written when a derived image is actually built (`dce new`, `dce rebuild-image`), not on a plain `dce rebuild-container` (which does not build) or base-only projects. Snapshot/restore events also append their own entries (`action: "snapshot"` / `"restore"`) so the log stays honest about images that came from a `dce snapshot` rather than a scope-derived build.
